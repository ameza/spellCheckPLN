...
"""
    ...
    def loadCorpus(self):
        ...
        corpusfile = open('corpusCR.data', 'r')
    ...
    def sentenceprobability(self, sent, gram='uni', form='antilog'):
        ...
        if gram == 'bi':
            for index, item in enumerate(words):
                ...
                P += self.probability(item, item + ' ' + words[index + 1], 'bi')
        if gram == 'tri':
            for index, item in enumerate(words):
                if index == len(words)- 2: break
                P += self.probability(item + ' ' + words[index + 1],
                                      item + ' ' + words[index + 1] + ' ' + words[index + 2], 'tri')
        if gram == 'quadri':
            for index, item in enumerate(words):
                if index == len(words)- 3: break
                P += self.probability(item + ' ' + words[index + 1] + ' ' + words[index + 2],
                                      item + ' ' + words[index + 1] + ' ' + words[index + 2] + ' ' + words[index + 3],
                                      'quadri')
        if gram == 'penti':
            for index, item in enumerate(words):
                if index == len(words)- 4: break
                P += self.probability(item + ' ' + words[index + 1] + ' ' + words[index + 2] + ' ' + words[index + 3],
                                      item + ' ' + words[index + 1] + ' ' + words[index + 2] + ' ' + words[
                                          index + 3] + ' ' + words[index + 4], 'penti')
        if form == 'log':
            return P
        elif form == 'antilog':
